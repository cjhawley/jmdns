...
public class HostInfo implements DNSStatefulObject {
  ...
  public static HostInfo newHostInfo(InetAddress address, JmDNSImpl dns, String jmdnsName) {
    ...
    try {
      ...
      logger.warn(
          "Could not initialize the host network interface on " + address + "because of an error: "
              + e.getMessage(), e);
  ...
  public boolean conflictWithRecord(DNSRecord.Address record) {
    DNSRecord.Address hostAddress = this
        .getDNSAddressRecord(record.getRecordType(), record.isUnique(), DNSConstants.DNS_TTL);
    if (hostAddress != null) {
  ...
  synchronized String incrementHostName() {
    _name = NameRegister.Factory.getRegistry()
        .incrementName(this.getInetAddress(), _name, NameRegister.NameType.HOST);
    return _name;
  }
