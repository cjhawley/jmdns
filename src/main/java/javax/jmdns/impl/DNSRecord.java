...
public abstract class DNSRecord extends DNSEntry {
  ...
  DNSRecord(String name, DNSRecordType type, DNSRecordClass recordClass, boolean unique, int ttl) {
    ...
    _isStaleAndShouldBeRefreshedPercentage =
        DNSConstants.STALE_REFRESH_STARTING_PERCENTAGE + _randomStaleRefreshOffset;
  }
   ...
   *
   * @return Returns true if a conflict with one of the services registered with JmDNS or with the
   * hostname occured.
   */
   ...
   * @return <code>true</code> is the record is stale and should be refreshed, <code>false</code>
   * otherwise.
   */
  ...
  * Increment the percentage that determines whether a record needs to be refreshed.
  ...
  public static abstract class Address extends DNSRecord {
    ...
    boolean handleQuery(JmDNSImpl dns, long expirationTime) {
      if (dns.getLocalHost().conflictWithRecord(this)) {
        DNSRecord.Address localAddress = dns.getLocalHost()
            .getDNSAddressRecord(this.getRecordType(), this.isUnique(), DNSConstants.DNS_TTL);
    ...
    @Override
    public ServiceInfo getServiceInfo(boolean persistent) {
      ServiceInfoImpl info = new ServiceInfoImpl(this.getQualifiedNameMap(), 0, 0, 0, persistent,
          (byte[]) null);
  ...
  public static class Service extends DNSRecord {
    ...
    boolean sameValue(DNSRecord other) {
      ...
      return (_priority == s._priority) && (_weight == s._weight) && (_port == s._port) && _server
          .equals(s._server);
    }
    ...
    boolean handleQuery(JmDNSImpl dns, long expirationTime) {
      ...
      if (info != null && (info.isAnnouncing() || info.isAnnounced()) && (_port != info.getPort()
          || !_server.equalsIgnoreCase(dns.getLocalHost().getName()))) {
        ...
        DNSRecord.Service localService = new DNSRecord.Service(info.getQualifiedName(),
            ...
            info.getPriority(), info.getWeight(), info.getPort(), dns.getLocalHost().getName());
        ...
        if (info.isProbing() && comparison > 0) {
          ...
          info.setName(NameRegister.Factory.getRegistry()
              .incrementName(dns.getLocalHost().getInetAddress(), info.getName(),
    ...
    boolean handleResponse(JmDNSImpl dns) {
      ServiceInfoImpl info = (ServiceInfoImpl) dns.getServices().get(this.getKey());
      if (info != null && (_port != info.getPort() || !_server
   ...
   *
   * @return <code>false</code> if this record can have multiple values in the cache,
