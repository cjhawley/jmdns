 ...
 * <p/>
 * The ServiceResolver will run only if JmDNS is in state ANNOUNCED. REMIND: Prevent having multiple
 * service resolvers for the same type in the timer queue.
 */
  ...
  protected DNSOutgoing addAnswers(DNSOutgoing out) throws IOException {
    ...
    for (ServiceInfo info : this.getDns().getServices().values()) {
      newOut = this.addAnswer(newOut,
          new DNSRecord.Pointer(info.getType(), DNSRecordClass.CLASS_IN, DNSRecordClass.NOT_UNIQUE,
