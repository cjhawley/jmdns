 ...
 * {@link javax.jmdns.impl.DNSStatefulObject} interface and therefore participate in the state
 * machine.
 *
...
public abstract class DNSStateTask extends DNSTask {
  ...
  /**
   * Associate the DNS host and the service infos with this task if not already associated and in
   * the same state.
   *
    ...
    try {
      ...
      synchronized (this.getDns()) {
        if (this.getDns().isAssociatedWithTask(this, this.getTaskState())) {
          logger1.debug(
              this.getName() + ".run() JmDNS " + this.getTaskDescription() + " " + this.getDns()
                  .getName());
      ...
      for (ServiceInfo serviceInfo : this.getDns().getServices().values()) {
        ...
        synchronized (info) {
          if (info.isAssociatedWithTask(this, this.getTaskState())) {
            logger1.debug(this.getName() + ".run() JmDNS " + this.getTaskDescription() + " " + info
                .getQualifiedName());
      ...
      if (!out.isEmpty()) {
        logger1.debug(this.getName() + ".run() JmDNS " + this.getTaskDescription() + " #" + this
            .getTaskState());
