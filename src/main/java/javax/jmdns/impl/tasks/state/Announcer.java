...
/**
 * The Announcer sends an accumulated query of all announces, and advances the state of all
 * serviceInfos, for which it has sent an announce. The Announcer also sends announcements and
 * advances the state of JmDNS itself.
 * <p/>
...
public class Announcer extends DNSStateTask {
  ...
  public void start(Timer timer) {
    if (!this.getDns().isCanceling() && !this.getDns().isCanceled()) {
      timer
          .schedule(this, DNSConstants.ANNOUNCE_WAIT_INTERVAL, DNSConstants.ANNOUNCE_WAIT_INTERVAL);
    }
  }
