 ...
 * the host name. The state of each service info of the host name is advanced, when a probe has been
 * sent for it. When the prober has run three times, it launches an Announcer.
 * <p/>
 * If a conflict during probes occurs, the affected service infos (and affected host name) are taken
 * away from the prober. This eventually causes the prober to cancel itself.
 */
public class Prober extends DNSStateTask {
  ...
  public void start(Timer timer) {
    ...
    if (now - this.getDns().getLastThrottleIncrement()
  ...
  protected DNSOutgoing buildOutgoingForInfo(ServiceInfoImpl info, DNSOutgoing out)
    ...
    newOut = this.addAuthoritativeAnswer(newOut,
        new DNSRecord.Service(info.getQualifiedName(), DNSRecordClass.CLASS_IN,
            DNSRecordClass.NOT_UNIQUE, this.getTTL(), info.getPriority(), info.getWeight(),
            info.getPort(), this.getDns().getLocalHost()
            .getName()));
    return newOut;
  }
