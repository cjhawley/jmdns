  ...
  public void run() {
    try {
      ...
      while (!this._jmDNSImpl.isCanceling() && !this._jmDNSImpl.isCanceled()) {
        ...
        if (this._jmDNSImpl.isCanceling() || this._jmDNSImpl.isCanceled() || this._jmDNSImpl
            .isClosing() || this._jmDNSImpl.isClosed()) {
          break;
        }
        try {
          ...
          if (msg.isValidResponseCode()) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                  this.getName() + ".run() JmDNS in message with error code:" + msg.print(true));
            }
          }
        } catch (IOException e) {
