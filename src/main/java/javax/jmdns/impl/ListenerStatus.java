...
/**
 * This class track the status of listener.<br/> The main purpose of this class is to collapse
 * consecutive events so that we can guarantee the correct call back sequence.
 *
...
public class ListenerStatus<T extends EventListener> {
  ...
  public static class ServiceListenerStatus extends ListenerStatus<ServiceListener> {
    ...
    /**
     * A service has been added.<br/> <b>Note:</b>This event is only the service added event. The
     * service info associated with this event does not include resolution information.<br/> To get
     * the full resolved information you need to listen to {@link #serviceResolved(ServiceEvent)} or
     * call {@link JmDNS#getServiceInfo(String, String, long)}
     *
     * <pre>
     *  ServiceInfo info = event.getDNS().getServiceInfo(event.getType(), event.getName())
     * </pre>
     * <p> Please note that service resolution may take a few second to resolve. </p>
     *
     ...
     *
     * @param event The ServiceEvent providing the name, the fully qualified type of the service,
     * and the service info record.
     */
    ...
    private static final boolean _sameInfo(ServiceInfo info, ServiceInfo lastInfo) {
      ...
      boolean isEqual = Arrays.equals(text, lastText);

      return isEqual && info.hasSameAddresses(lastInfo);
    }
  ...
  public static class ServiceTypeListenerStatus extends ListenerStatus<ServiceTypeListener> {
     ...
     * </pre>
     *
     * @param event The service event providing the fully qualified type of the service with
     * subtype.
     */
    void subTypeForServiceTypeAdded(ServiceEvent event) {
      if (null == _addedTypes.putIfAbsent(event.getType(), event.getType())) {
        ...
        logger
            .trace("Service Sub Type Added called for a service sub type already added: " + event);
      }
    }
  ...
  public boolean equals(Object obj) {
    return (obj instanceof ListenerStatus) && this.getListener()
        .equals(((ListenerStatus<?>) obj).getListener());
  }
