...
public class DNSQuestion extends DNSEntry {
  ...
  private static class DNS4Address extends DNSQuestion {
    ...
    public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
      DNSRecord answer = jmDNSImpl.getLocalHost()
          .getDNSAddressRecord(this.getRecordType(), DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL);
      if (answer != null) {
        answers.add(answer);
      }
    }
    ...
    public boolean iAmTheOnlyOne(JmDNSImpl jmDNSImpl) {
      ...
      return jmDNSImpl.getLocalHost().getName().equals(name) || jmDNSImpl.getServices().keySet()
          .contains(name);
    }
    ...
    public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
      ...
      if (this.isServicesDiscoveryMetaQuery()) {
        for (String serviceType : jmDNSImpl.getServiceTypes().keySet()) {
          ...
          answers.add(
              new DNSRecord.Pointer("_services._dns-sd._udp.local.", DNSRecordClass.CLASS_IN,
                  DNSRecordClass.NOT_UNIQUE, DNSConstants.DNS_TTL, typeEntry.getType()));
        }
  ...
  private static class Text extends DNSQuestion {
    ...
    public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
      this.addAnswersForServiceInfo(jmDNSImpl, answers,
          (ServiceInfoImpl) jmDNSImpl.getServices().get(this.getName().toLowerCase()));
    }
  ...
  boolean answeredBy(DNSEntry rec) {
    return this.isSameRecordClass(rec) && this.isSameType(rec) && this.getName()
        .equals(rec.getName());
  }
  ...
  protected void addAnswersForServiceInfo(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers,
    ...
    if ((info != null) && info.isAnnounced()) {
      if (this.getName().equalsIgnoreCase(info.getQualifiedName()) || this.getName()
        ...
        answers.addAll(
            info.answers(this.getRecordClass(), DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL,
                jmDNSImpl.getLocalHost()));
      }
      if (logger.isDebugEnabled()) {
        logger.debug(jmDNSImpl.getName() + " DNSQuestion(" + this.getName()
   ...
   *
   * @param jmDNSImpl DNS holding the records
   * @return <code>true</code> if we are the only one with the answer to the question,
