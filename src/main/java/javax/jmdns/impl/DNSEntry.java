...
public abstract class DNSEntry {
  ...
  DNSEntry(String name, DNSRecordType type, DNSRecordClass recordClass, boolean unique) {
    ...
    _type = (application.length() > 0 ? "_" + application + "." : "") + (protocol.length() > 0 ? "_"
        + protocol + "." : "") + domain + ".";
  ...
  public boolean isSameEntry(DNSEntry entry) {
    return this.getKey().equals(entry.getKey()) && this.matchRecordType(entry.getRecordType())
   ...
   *
   * @return <code>true</code> if the two entries have are for the same subtype, <code>false</code>
   * otherwise
   */
  ...
  public boolean matchRecordClass(DNSRecordClass recordClass) {
    return (DNSRecordClass.CLASS_ANY == recordClass) || (DNSRecordClass.CLASS_ANY == this
        .getRecordClass()) || this.getRecordClass().equals(recordClass);
  }
  ...
  public String getSubtype() {
    return StringUtils.trimToEmpty(this.getQualifiedNameMap().get(Fields.Subtype));
  }
  ...
  public String getName() {
    return StringUtils.trimToEmpty(_name);
  }
  ...
  public String getType() {
    return StringUtils.trimToEmpty(_type);
  }
  ...
  public boolean isServicesDiscoveryMetaQuery() {
    return _qualifiedNameMap.get(Fields.Application).equals("dns-sd") && _qualifiedNameMap
        .get(Fields.Instance).equals("_services");
  }
  ...
  public boolean isDomainDiscoveryQuery() {
    ...
    if (_qualifiedNameMap.get(Fields.Application).equals("dns-sd")) {
      ...
      return "b".equals(name) || "db".equals(name) || "r".equals(name) || "dr".equals(name) || "lb"
          .equals(name);
    }
    return false;
  }
  ...
  /**
   * Does a lexicographic comparison of the byte array representation of this record and that
   * record. This is needed for tie-break tests according to draft-cheshire-dnsext-multicastdns-04.txt
   * chapter 9.2.
   *
