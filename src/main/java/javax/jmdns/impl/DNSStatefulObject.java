...
/**
 * Sets of methods to manage the state machine.<br/> <b>Implementation note:</b> This interface is
 * accessed from multiple threads. The implementation must be thread safe.
 *
...
public interface DNSStatefulObject {
  ...
  /**
   * This class define a semaphore. On this multiple threads can wait the arrival of one event.
   * Thread wait for a maximum defined by the timeout. <p> Implementation note: this class is based
   * on {@link java.util.concurrent.Semaphore} so that they can be released by the timeout timer.
   * </p>
   *
   * @author Pierre Frisch
   */
  ...
  public static class DefaultImplementation extends ReentrantLock implements DNSStatefulObject {
      ...
      if (this._task == task) {
        ...
        try {
          if (this._task == task) {
            ...
            logger.warn("Trying to advance state whhen not the owner. owner: " + this._task
                + " perpetrator: " + task);
          }
        } finally {
          this.unlock();
    ...
    public String toString() {
      try {
        return (_dns != null ? "DNS: " + _dns.getName() + " [" + _dns.getInetAddress() + "]"
            : "NO DNS") + " state: " + _state + " task: " + _task;
   ...
   * @param task associated task
   * @param state state of the task
   * @return <code>true</code> is the task is associated with this object, <code>false</code>
   * otherwise.
   */
  public boolean isAssociatedWithTask(DNSTask task, DNSState state);
