...
public abstract class JmDNS implements Closeable {
  ...
  static {
    try {
      InputStream inputStream = JmDNS.class.getClassLoader()
  ...
  /**
   * <p> Create an instance of JmDNS. </p> <p> <b>Note:</b> This is a convenience method. The
   * preferred constructor is {@link #create(InetAddress, String)}.<br/> Check that your platform
   * correctly handle the default localhost IP address and the local hostname. In doubt use the
   * explicit constructor.<br/> This call is equivalent to <code>create(null, null)</code>. </p>
   *
  ...
  /**
   * <p> Create an instance of JmDNS and bind it to a specific network interface given its
   * IP-address. </p> <p> <b>Note:</b> This is a convenience method. The preferred constructor is
   * {@link #create(InetAddress, String)}.<br/> Check that your platform correctly handle the
   * default localhost IP address and the local hostname. In doubt use the explicit
   * constructor.<br/> This call is equivalent to <code>create(addr, null)</code>. </p>
   *
   ...
   * preferred constructor is {@link #create(InetAddress, String)}.<br/> Check that your platform
   * correctly handle the default localhost IP address and the local hostname. In doubt use the
   * explicit constructor.<br/> This call is equivalent to <code>create(null, name)</code>. </p>
   *
  ...
  /**
   * <p> Create an instance of JmDNS and bind it to a specific network interface given its
   * IP-address. </p> If <code>addr</code> parameter is null this method will try to resolve to a
   * local IP address of the machine using a network discovery: <ol> <li>Check the system property
   * <code>net.mdns.interface</code></li> <li>Check the JVM local host</li> <li>Use the {@link
   * NetworkTopologyDiscovery} to find a valid network interface and IP.</li> <li>In the last resort
   * bind to the loopback address. This is non functional in most cases.</li> </ol> If
   * <code>name</code> parameter is null will use the hostname. The hostname is determined by the
   * following algorithm: <ol> <li>Get the hostname from the InetAdress obtained before.</li> <li>If
   * the hostname is a reverse lookup default to <code>JmDNS name</code> or <code>computer</code> if
   * null.</li> <li>If the name contains <code>'.'</code> replace them by <code>'-'</code></li>
   * <li>Add <code>.local.</code> at the end of the name.</li> </ol> <p> <b>Note:</b> If you need to
   * use a custom {@link NetworkTopologyDiscovery} it must be setup before any call to this method.
   * This is done by setting up a {@link NetworkTopologyDiscovery.Factory.ClassDelegate} and
   * installing it using {@link NetworkTopologyDiscovery.Factory#setClassDelegate(NetworkTopologyDiscovery.Factory.ClassDelegate)}.
   * This must be done before creating a {@link JmDNS} or {@link JmmDNS} instance. </p>
   *
   * @param addr IP address to bind to.
   * @param name name of the newly created JmDNS
   * @return jmDNS instance
   ...
   * @return Internet Address
   * @throws IOException if there is an error in the underlying protocol, such as a TCP error.
   * @deprecated do not use this implementation yields unpredictable results use {@link
   * #getInetAddress()}
   */
  @Deprecated
   ...
   * <p/>
   * Usage note: Do not call this method from the AWT event dispatcher thread. You will make the
  ...
  /**
   * Get service information. If the information is not cached, the method will block for the given
   * timeout until updated information is received.
   * <p/>
   * Usage note: If you call this method from the AWT event dispatcher thread, use a small timeout,
   * or you will make the user interface unresponsive.
   *
   ...
   * @param type fully qualified service type, such as <code>_http._tcp.local.</code> .
   * @param name unqualified service name, such as <code>foobar</code> .
   * @param persistent if <code>true</code> ServiceListener.resolveService will be called whenever
   ...
   * the service may be changed to make it unique.<br> Note that the given {@code ServiceInfo} is
   * bound to this {@code JmDNS} instance, and should not be reused for any other {@linkplain
  ...
  /**
   * Unregister a service. The service should have been registered. <p> <b>Note:</b> Unregistered
   * services will not disappear form the list of services immediately. According to the
   * specification, when unregistering services we send goodbye packets and then wait <b>1s</b>
   * before purging the cache.<br/> This is support for shared records that can be rescued by some
   * other cooperation DNS.
   *
   * <pre>
   * Clients receiving a Multicast DNS Response with a TTL of zero SHOULD NOT immediately delete the
   * record from the cache, but instead record a TTL of 1 and then delete the record one second
   * later.
   * </pre>
   *
   * </p>
   ...
   * be notified of the new service type. <p> Service types are automatically registered as they are
   * discovered. </p>
   *
   * @param type full qualified service type, such as <code>_http._tcp.local.</code>.
   * @return <code>true</code> if the type or subtype was added, <code>false</code> if the type was
   * already registered.
   */
   ...
   * not register a subtype is listed in the empty subtype section.
   *
   * @param type Service type name, such as <code>_http._tcp.local.</code>.
